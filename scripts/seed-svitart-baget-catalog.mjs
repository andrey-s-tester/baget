...
async function main() {
  const raw = await readFile("data/svitart-baget-catalog.json", "utf8");
  const items = JSON.parse(raw);
  if (!Array.isArray(items)) {
    throw new Error("data/svitart-baget-catalog.json must be an array");
  }
  ...
  console.log(`Upserted ${upserted} frame profiles from SvitArt (catalogSource=svitart_net)`);
}
